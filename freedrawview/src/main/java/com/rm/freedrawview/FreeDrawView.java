...
public class FreeDrawView extends View implements View.OnTouchListener {
    ...
    public boolean isDrawing() {
        return mPaths.size() > 0 || mPoints.size() > 0 || mCanceledPaths.size() > 0;
    }
    ...
    public void getDrawScreenshot(@NonNull final DrawCreatorListener listener) {
        new TakeScreenShotAsyncTask(this, listener).execute();
    }
    ...
    protected void onDraw(Canvas canvas) {
        if (mBitmap == null && mPaths.size() == 0 && mPoints.size() == 0) {
            return;
        }
    ...
    private void drawBitmapBg(Canvas canvas) {
        if (mBitmap == null) {
            return;
        }
        int viewWidth = getWidth();
        int viewHeight = getHeight();
        int bgWidth = mBitmap.getWidth();
        int bgHeight = mBitmap.getHeight();
        float horizontalOffset = (viewWidth - bgWidth) / 2.0f;
        float verticalOffset = (viewHeight - bgHeight) / 2.0f;
        canvas.drawBitmap(mBitmap, horizontalOffset, verticalOffset, mCurrentPaint);
    }

    private void drawHistoryPaths(Canvas canvas) {
...
//            }
            ...
            Point current = mPoints.get(0);
            mCurrentPath.moveTo(current.x, current.y);
            Point next = mPoints.get(1);
    ...
    public boolean onTouch(View view, MotionEvent motionEvent) {
        ...
        if ((motionEvent.getAction() != MotionEvent.ACTION_UP) &&
            ...
            for (int i = 0; i < motionEvent.getHistorySize(); i++) {
                tX = motionEvent.getHistoricalX(i);
                tY = motionEvent.getHistoricalY(i);
                if (isOutEdge(tX, tY)) {
                    continue;
                }
                point = new Point();
    ...
    private boolean isOutEdge(float x, float y) {
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        rightEdge = w - getPaddingRight();
        topEdge = getPaddingTop();
        bottomEdge = h - getPaddingBottom();
    ...
    public void setBitmapAsBg(Bitmap bitmap) {
        if (bitmap == null) {
            return;
        }
        this.mBitmap = bitmap;
    ...
    private static class TakeScreenShotAsyncTask extends AsyncTask<Void, Void, Void> {
        ...
        private WeakReference<FreeDrawView> freeDrawViewWeakReference;

        public TakeScreenShotAsyncTask(FreeDrawView freeDrawView, @NonNull DrawCreatorListener listener) {
            freeDrawViewWeakReference = new WeakReference<>(freeDrawView);
            mListener = listener;
        }
        ...
        protected void onPreExecute() {
            super.onPreExecute();
            FreeDrawView freeDrawView = freeDrawViewWeakReference.get();
            if (freeDrawView == null) {
                cancel(true);
                return;
            }
            mWidth = freeDrawView.getWidth();
            mHeight = freeDrawView.getHeight();
        }
        ...
        protected void onPostExecute(Void aVoid) {
            ...
            freeDrawView.drawPaths(mCanvas);
            freeDrawView.setBitmapAsBg(mBitmap);
